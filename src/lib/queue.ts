...
export type JobHandler<P = any, E extends AnyEnv = AnyEnv> =
  (payload: P, ctx: JobContext<E>) => Promise<void> | void;
...
export function defineJobs<E extends AnyEnv = AnyEnv>(registry: JobRegistry<E>) {
  return registry;
}

export function createQueueConsumer<E extends AnyEnv = AnyEnv>(registry: JobRegistry<E>) {
  return {
